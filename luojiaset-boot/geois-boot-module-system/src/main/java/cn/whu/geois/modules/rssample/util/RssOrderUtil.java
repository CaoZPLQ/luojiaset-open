...
public class RssOrderUtil {
    ...
    /**
     * 根据id进行加密+加随机数组成固定长度编码,适用于字符串型id
     */
    private static String toCode(String userId) {
        String idStr =userId.replaceAll("[^0-9]","");
        System.out.println("idStr:"+idStr);
        if (idStr.length()>10){
            idStr =idStr.substring(0,9);
        }
        System.out.println("idStr:"+idStr);
        StringBuilder idsbs = new StringBuilder();
        for (int i = idStr.length() - 1; i >= 0; i--) {
    ...
    /**
     * 生成不带类别标头的编码,适用于字符串型id
     *
     * @param userId
     */
    private  synchronized String getCode(String userId) {
        userId = userId == null ? "10000" : userId;
        return getDateTime() + toCode(userId);
    }
    ...
    /**
     * 生成订单单号编码(调用方法)
     * @param userId  网站中该用户唯一ID 防止重复,适用于字符串型id
     */
