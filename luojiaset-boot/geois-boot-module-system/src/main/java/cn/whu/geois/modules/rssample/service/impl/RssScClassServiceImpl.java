...
import cn.whu.geois.modules.rssample.mapper.RssScClassMapper;
...
@Service
@DS("postgres")
public class RssScClassServiceImpl extends ServiceImpl<RssScClassMapper, RssScClass> implements IRssScClassService {
    @Transactional(rollbackFor = Exception.class)
    @Override
    public boolean insertBatch(List<RssScClass> entityList){
        return insertBatch(entityList,1000);
    }
    @Transactional(rollbackFor = Exception.class)
    @Override
    public boolean insertBatch(List<RssScClass> entityList,int batchSize){
